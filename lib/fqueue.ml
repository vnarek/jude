...
let is_empty = function[@warning "-4"] Shallow Zero -> true | _ -> false
...
let rec take_front_exn : 'a. 'a t -> 'a * 'a t =
 fun q ->
  ...
  | Deep (n, One x, (lazy q'), tail) ->
      if is_empty q' then (x, Shallow tail)
      else
...
let rec take_back_exn : 'a. 'a t -> 'a t * 'a =
 fun q ->
  ...
  | Deep (n, hd, (lazy q'), One x) ->
      if is_empty q' then (Shallow hd, x)
      else
...
let rec nth_exn : 'a. int -> 'a t -> 'a =
 fun i q ->
  ...
  | _, Deep (_, l, q, r) ->
      if i < _size_digit l then _nth_digit i l
      else
...
let _digit_to_seq : type l. ('a, l) digit -> 'a sequence =
 fun d k ->
  match d with
  | Zero -> ()
  | One x -> k x
...
let _digit_to_klist : type l. ('a, l) digit -> 'a klist -> 'a klist =
 fun d cont ->
  match d with
  | Zero -> _nil
...
let to_klist q =
  let rec aux : 'a. 'a t -> 'a klist -> 'a klist =
   fun q cont () ->
    ...
    | Deep (_, hd, (lazy q'), tl) ->
        _digit_to_klist hd
          (_flat_klist (aux q' _nil) (_digit_to_klist tl cont))
          ()
  in
  aux q _nil
...
let rec _equal_klist eq l1 l2 =
  match (l1 (), l2 ()) with
  | `Nil, `Nil -> true
  | `Nil, _ | _, `Nil -> false
  | `Cons (x1, l1'), `Cons (x2, l2') -> eq x1 x2 && _equal_klist eq l1' l2'
...
let ( -- ) a b =
  let rec up_to q a b = if a = b then snoc q a else up_to (snoc q a) (a + 1) b
  and down_to q a b =
    if a = b then snoc q a else down_to (snoc q a) (a - 1) b
  in
...
let ( --^ ) a b =
  if a = b then empty else if a < b then a -- (b - 1) else a -- (b + 1)
